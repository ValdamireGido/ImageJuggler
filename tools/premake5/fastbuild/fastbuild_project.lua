...
local function compile(indentlevel, prj, cfg, commonbasepath)
	...
	if type(cfg.excludes) == "table" then
	...
	_p(indentlevel, ".CompilerOutputPath = '%s'", add_trailing_backslash(cfg.objdir))
...
local function library(prj, cfg, useconfig, commonbasepath)
	...
	_p(1, "Library('%s-%s-%s')", prj.name, cfg.buildcfg, cfg.platform)
	_p(1, '{')
	...
	local dir = os.mkdir(path.getdirectory(cfg.buildtarget.fullpath))
...
local function binary(prj, cfg, useconfig, bintype, commonbasepath)
	_p(1, "ObjectList('%s_obj-%s-%s')", prj.name, cfg.buildcfg, cfg.platform)
	_p(1, '{')
	...
	_p(1, "%s('%s-%s-%s')", bintype, prj.name, cfg.buildcfg, cfg.platform)
	_p(1, '{')

	useconfig(2)
	_p(2, '.Libraries = {')
	_p(3, "'%s_obj-%s-%s',", prj.name, cfg.buildcfg, cfg.platform) -- Refer to the ObjectList

	local sorteddeplibs = {}
	for _, deplib in ipairs(premake.config.getlinks(cfg, "dependencies", "basename")) do
		table.insert(sorteddeplibs, string.format("'%s-%s-%s',", deplib, cfg.buildcfg, cfg.platform))
	end
	...
	local deffile = premake.config.findfile(cfg, ".def")
	if deffile then
	...
	table.insert(linkeroptions, '/INCREMENTAL'..((premake.config.canLinkIncremental(cfg) and '') or ':NO'))

	for _, addloption in ipairs(cfg.linkoptions) do
		table.insert(linkeroptions, addloption)
	end

	local linklibs = premake.config.getlinks(cfg, "all", "fullpath")
	...
	os.mkdir(cfg.buildtarget.fullpath)
...
local function alias(prj, cfg, target)
	_p(1, "Alias('%s-%s-%s')", prj.name, cfg.buildcfg, cfg.platform)
	_p(1, '{')
...
function premake.fastbuild.project(prj)
	...
	local file_tree = premake.project.getsourcetree(prj) 
	premake.tree.traverse(file_tree, {
		onnode = function(file)
			if path.iscppfile(file.name) then				
				table.insert(cppfiles, file.relpath)
			end				
		end	
	})  
	...
	for cfg in premake.project.eachconfig(prj, platform) do
